...
Xamarin.Forms weist eine andere Projektstruktur auf als Flutter,  das nur mit einem Projekt arbeitet.  Wärend das Flutter Projekt alle notwendigen Inhalte für iOS und Android inkludiert, \footcite[Vgl.][S. 113]{Biessek2019} setzt sich die sogenannte Lösung (eng.  Solution) bei Xamarin.Forms aus mehreren Projekten zusammen.  Es gibt für jede Plattform ein dediziertes Projekt, das den plattformspezifischen Code,  Konfigurationen und Icons beinhaltet, sowie ein Projekt für den plattformunabhängigen Quelltext.   \footcite[Vgl.][S. 25f.]{Petzold2016} Icons und Konfigurationen werden bei Flutter in einem gleichen oder ähnlichen Format und nur in einem weiteren Projekt hinterlegt und lassen sich folglich migrieren.  In den Ausschlusskriterien in Kapitel \ref{chap:CompilerEntwurf} dieser Arbeit wurde bereits der plattformspezifische Quelltext von Xamarin.Forms für die Übersetzung exkludiert,  der Vollständigkeit  halber wird er in diesem Abschnitt dennoch erwähnt. 


\subsection{Metadaten}
Zu den Metadaten einer Anwendungen gehört unter anderem der Name der Anwendung,  Informationen wie die benötigte Betriebssystemversion, das \glq App Launcher Icon\grq{}  das auf dem Smartphonebildschirm angezeigt wird und die Berechtigungen,  die von der mobile Anwendung während der Ausführung beantragen werden können.  Diese Eigenschaften werden in Xamarin.Forms innerhalb der nativen Projekte wie folgt definiert.  In iOS können Änderungen innerhalb der Datei \glq Info.plist\grq{}  definiert werden. \footcite[Vgl.][Abgerufen am \today]{MicrosoftInfoPlist2017} In Android kann \glq AndroidManifest.xml\grq{}  zugegriffen werden, um diese Änderungen durchzuführen. \footcite[Vgl.][Abgerufen am \today]{MicrosoftManifest2018} Flutter verwendet für die Definition der Metadaten die identischen Dateien,  daher ist es möglich,  diese aus dem Xamarin.Forms Projekt zu kopieren und innerhalb des Flutter Projektes zu speichern.  Dafür muss die \glq Info.plist\grq-Datei im Verzeichnis \glq ios/Runner/\grq{} und die \glq AndroidManifest.xml\grq{}  in \glq android/app/src/main/\grq{} gespeichert werden.  In diesen Dateien wird außerdem der Identifizierer der Anwendung definiert.  Durch eine Kopie der Konfigurationsdatei und Erhöhung der Versionsnummer wird eine spätere Kompilierung der Flutter-App demnach als eine Aktualisierung der Xamarin.Forms App erkannt.  \footcite[Vgl.][Abgerufen am \today]{Rana2020}


\subsection{Bilder und Startbildschirm}
Innerhalb von Apps können Bilder das Benutzererlebnis verbessern und helfen,  eine Aktion zu veranschaulichen oder komplexe Botschaften zu verdeutlichen.  \footcite[Vgl.][Abgerufen am \today]{GoogleMaterialImages2020} In iOS und Android können sie in verschiedenen Auflösungen bereit gestellt werden,  das Betriebssystem wählt während der Laufzeit das beste Bild basierend auf den Eigenschaften des Smartphonedisplays aus.  Xamarin.Forms verwendet die \acfp{api} der nativen Plattformen zum Laden lokaler Bilder und unterstützt daher die plattformspezifischen Funktionalitäten.\footcite[Vgl.][Abgerufen am \today]{MicrosoftXamImages2020} Zur Verwendung nativer Bilddateien müssen die Bilder in Xamarin.Forms zu jedem Anwendungsprojekt hinzugefügt werden und vom gemeinsamen Xamarin.Forms-Code referenziert werden.  Flutter verwendet im Gegensatz zu Xamarin.Forms keine nativen \acp{api},  sondern ein einfaches dichte-basiertes Format,  ähnlich dem von iOS.  Für die Anzeige von Bildern arbeitet Flutter mit sogenannten logischen Pixeln.  Alle Bilderressourcen können sich in einem beliebigen Ordner innerhalb des Projektes befinden, da Flutter keine vordefinierte Ordnerstrukturen hat. \footcite[Vgl.][Abgerufen am \today]{GoogleFlutterImages2020} Der Startbildschirm (engl. Splashscreen) ist eine Ansicht,  die Angezeigt wird,  während die App gestartet wird.  Dieser Ladebildschrim wird ebenfalls in den Plattformspezifischen Projekten gespeichert und kann ähnlich wie die \glq Info.plist\grq{}- und \glq AndroidManifest.xml\grq{}-Dateien von dem Xamarin.Forms Projekt in das Flutter Projekt kopiert werden, da die technische Implementierung identisch ist. \footcite[Vgl.][Abgerufen am \today]{GoogleSplash2020} 
...
In\footcitetext[Quelltext in Anlehnung an][Abgerufen am \today]{GoogleFlutterFonts2020}
 den Ausschlusskriterien in Kapitel \ref{chap:CompilerEntwurf} dieser Arbeit wurde bereits der plattformspezifische Quelltext von Xamarin.Forms für die Übersetzung exkludiert,  der Vollständigkeit sollen die Unterschiede zwischen den Plattformen jedoch trotzdem erwähnt werden.  Xamarin.Forms unterteilt plattformspezifischer Quelltext in zwei Kategorien, den sogenannten \glq DependencyService\grq{} die es erlauben native plattformfunktionenalitäten aus dem geteilten Code aufzurufen. \footcite[Vgl.][Abgerufen am \today]{MicrosoftDependencyService2017}  Sowie \glq Custom Renderers\grq,  die es ermöglichen das Aussehen und Verhalten von Xamarin.Forms Steuerlementen auf den einzelnen Plattformen anzupassen. \footcite[Vgl.][Abgerufen am \today]{MicrosoftCustomRenderers2017}.
In Flutter wird für die Ausführung von nativen Code auf \glq platform-channels\grq{} zurückgegriffen die über sogenannte channels mit dem \ac{ui} kommunizieren. \footcite[Vgl.][Abgerufen am \today]{GooglePlatformspecificCode2020} Für \glq Custom Renderer\grq{}  gibt es bei Flutter jedoch keine alternative, da Flutter keine nativen Steuerelemente sondern Widgets anzeigt auf die im späteren Verlaufe Kapitels detailliert eingegangen wird.  
...
\subsection{Interkation mit der Hardware}
...
Innerhalb von \glq Stacklayouts\grq{} können Xamarin.Forms Steuerelemente mit Hilfe der Eigenschaften \glq HorzintalOptions\grq{} und \glq VerticalOptions\grq{}  ausgerichtet werden.  Für diese Eigenschaften können die folgenden Werte gesetzt werden: \footcite[Vgl.][Abgerufen am \today]{MicrosoftLayouts2020} 

\begin{itemize}
\setlength\itemsep{-0.6em}
 \item \glq Start\grq{}: positioniert die Ansicht bei einer horizontalen Ausrichtung an der linken Seite des übergeordneten Layouts, und bei vertikaler Ausrichtung positioniert sie die Ansicht am oberen Rand des übergeordneten Layouts.
  \item \glq Center\grq{}: zentriert die Ansicht in der Mitte. 
  \item \glq End\grq{}: Bei horizontaler Ausrichtung wird die Ansicht auf der rechten Seite des übergeordneten Layouts und bei vertikaler Ausrichtung  unteren Rand des übergeordneten Layouts positioniert.
  \item \glq Fill\grq{}:  Bei einer horizontalen Ausrichtung sorgt Fill dafür, dass die Ansicht die Breite des übergeordneten Layouts ausfüllt, und bei der vertikalen Ausrichtung sorgt es dafür, dass die Ansicht die Höhe des übergeordneten Layouts ausfüllt.
\end{itemize}
Neben diesen Werten stehen auch noch Ausprägungen mit der Erweiterung \glq expand" zur Verfügung. Diese Werte \glq StartAndExpand\grq{} , \glq CenterAndExpand\grq{} , \glq EndAndExpand\grq{}  und \glq FillAndExpand\grq{}  werden verwendet, um die Ausrichtungspräferenz festzulegen und um zu bestimmen, ob die Ansicht mehr Platz einnimmt, wenn dieser im übergeordneten \glq StackLayout\grq{} verfügbar ist. \footcite[Vgl.][Abgerufen am \today]{MicrosoftLayouts2020} 
...
Nach der Einführung in allen verfügbaren Seiten,  Layouts und Steuerelemente soll an dieser Stelle exemplarisch ein UI von  Xamarin.Forms zu Flutter überführt werden.   Als Beispielseite wurde die in \ref{fig:ExamplePage} dargestellte Login-Seite mithilfe von Xamarin.Forms entwickelt.  
...
Die abgebildete Login-Seite zeigt eine zentrierte Form mit zwei Eingabefeldern für Benutzernamen und Passwort und einer Schaltfläche für die Ausführung des Loginvorganges.  Basierend auf den in diesem Abschnitt ermittelten Gegenüberstellungen von Steuerelementen und Widgets kann anschließend eine Transformation des Xamarin.Forms Layout-Baums zu dem Flutter Widget Tree durchgeführt werden.  Für diese Darstellung wird in Abbildung \ref{fig:LayoutTree} eine Baumstruktur verwendet,  welche die Verschachtlung innerhalb der Benutzeroberfläche visualisiert.  
\begin{figure}[!ht]
...
Ersichtlich wird aus dieser Abbildung,  dass in Xamarin.Forms das zentrale Element mithilfe des Steuerelementes \glq Grid\grq{} zentriert wurde.  Wie bereits beschrieben könnte dies auch mit einem \glq StackLayout\grq{} und der Ausrichtungsoption \glq CenterAndExpand\grq{} realisiert werden. Auch in Flutter könnte dieses Layout mit anderen Widgets Realisiert werden.  So könnte das äußere \glq GridView ebenfalls mit den Widgets \glq Row\grq{} und \glq Column\grq{} ersetzt werden.  Durch das austauschen von Steuerelementen und Widgets kann es unter Umständen vorkommen,  dass erzeugte Flutter Ansichten nicht optimal erzeugt wurden.  So steht für die Zentrierung von Elementen auch ein  Widget mit dem Namen \glq Center\grq{}  zur Verfügung,  welches beim Austausch von Widgets nicht berücksichtigt wird.  Der im Rahmen dieser Arbeit entwickelte Prototyp wird jedoch ausschließlich Elemente wie in diesem Abschnitt beschrieben austauschen. 
